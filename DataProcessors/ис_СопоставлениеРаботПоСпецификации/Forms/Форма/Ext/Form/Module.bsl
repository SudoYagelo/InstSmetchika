...
Процедура РезультатДеревомПровереноПриИзменении(Элемент)
	...
	Если НЕ Родитель = Неопределено Тогда
		Родитель.Проверено = ТекущиеДанные.Проверено;
	КонецЕсли;
...
Процедура РезультатДеревомЭтоРасходникПриИзменении(Элемент)
	...
	УстановитьПризнакЭтоРасходникиДляПодчиненных(ТекущиеДанные);
...
#Область ОбработкаФлагаРасходники

&НаКлиенте
Процедура УстановитьПризнакЭтоРасходникиДляПодчиненных(ТекущиеДанные)
	
	ПодчиненныеЭлементыДерева = ТекущиеДанные.ПолучитьЭлементы();
	
	Для Каждого ПодчиненныйЭлементДерева Из ПодчиненныеЭлементыДерева Цикл
		УстановитьПризнакЭтоРасходникРекурсивно(ПодчиненныйЭлементДерева, ТекущиеДанные.ЭтоРасходник);
	КонецЦикла;
	
КонецПроцедуры

&НаКлиенте
Процедура УстановитьПризнакЭтоРасходникРекурсивно(СтрокаДерева, ЗначениеЭтоРасходник)
	
	СтрокаДерева.ЭтоРасходник = ЗначениеЭтоРасходник;
	
	Для Каждого ПодчиненныйЭлемент Из СтрокаДерева.ПолучитьЭлементы() Цикл
		УстановитьПризнакЭтоРасходникРекурсивно(ПодчиненныйЭлемент, ЗначениеЭтоРасходник);
	КонецЦикла;
...
Функция ОбработатьСтрокуДереваДляДобавленияВЖурнал(СтрокаДерева)
	...
	// у головы не может быть ЗначениеЗаполнено(СтрокаДерева.КВР)
	Если СтрокаДерева.ДобавитьВЖурнал И ЗначениеЗаполнено(СтрокаДерева.КВР) Тогда
		
		ЗаписатьВКешОтветовИИ(СтрокаДерева);
...
Процедура ЗагрузитьТабличногоДокумента(ТабличныйДокумент)
	...
	Для каждого СтрокаТЗ Из ТаблицаДанных Цикл
		...
		Иначе   
			...
			СтрокаРезультат.ТипМарка = СтрокаТЗ.Тип_Марка___обозначение__документа___опросногоЛиста;
