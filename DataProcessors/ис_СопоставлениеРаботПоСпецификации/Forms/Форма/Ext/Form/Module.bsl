...
&НаКлиенте
Процедура РезультатДеревомЭтоРасходникПриИзменении(Элемент)
	...
	Если ТекущиеДанные.ЭтоРасходник Тогда
		ЗачиститьПоляДляРасходника(ТекущиеДанные);
	КонецЕсли;
...
Процедура ДобавитьВЖурнал(Команда)
	...
	КоличествоДобавлено = ДобавитьВЖурналНаСервере();

	Если КоличествоДобавлено > 0 Тогда
		ПоказатьПредупреждение(, СтрШаблон("Добавлено позиций в журнал: %1", КоличествоДобавлено));
	Иначе
		ПоказатьПредупреждение(, "Не выбрано ни одной позиции для добавления в журнал");
	КонецЕсли;
...
&НаКлиенте
Процедура ЗачиститьПоляДляРасходника(ТекущиеДанные)

	// Зачищаем поля, которые не нужны для расходников
	ТекущиеДанные.КВР = Неопределено;
	ТекущиеДанные.КодСтатьи = "";
	ТекущиеДанные.Уверенность = 0;
...
&НаСервере
Функция ДобавитьВЖурналНаСервере()

	КоличествоДобавлено = 0;

	// Обходим дерево результатов и ищем отмеченные позиции
	Для Каждого СтрокаВерхнегоУровня Из РезультатДеревом.ПолучитьЭлементы() Цикл
		КоличествоДобавлено = КоличествоДобавлено + ОбработатьСтрокуДереваДляДобавленияВЖурнал(СтрокаВерхнегоУровня);
	КонецЦикла;
...
&НаСервере
Функция ОбработатьСтрокуДереваДляДобавленияВЖурнал(СтрокаДерева)

	КоличествоДобавлено = 0;

	// Проверяем, отмечена ли текущая строка для добавления
	Если СтрокаДерева.ДобавитьВЖурнал И НЕ СтрокаДерева.ДобавленоВЖурнал Тогда

		// Проверяем, заполнены ли необходимые данные
		Если ЗначениеЗаполнено(СтрокаДерева.НаименованиеПозиции) И ЗначениеЗаполнено(СтрокаДерева.КВР) Тогда

			// Создаем новый элемент справочника
			НовыйЭлементСправочника = Справочники.ПравилаСопоставленияДляИИ.СоздатьЭлемент();

			// Заполняем реквизиты
			НовыйЭлементСправочника.Наименование = Лев(СтрокаДерева.НаименованиеПозиции, 25);

			// Формируем текст правила с информацией о сопоставлении
			ТекстПравила = СтрШаблон("Наименование: %1
				|КВР: %2
				|Головная номенклатура: %3
				|Уверенность: %4",
				СтрокаДерева.НаименованиеПозиции,
				СтрокаДерева.КВР,
				СтрокаДерева.ГоловнаяНоменклатура,
				СтрокаДерева.Уверенность);

			НовыйЭлементСправочника.ТекстПравила = ТекстПравила;

			// Получаем группу классификатора из КВР
			Если ЗначениеЗаполнено(СтрокаДерева.КВР) Тогда
				НовыйЭлементСправочника.ГруппаКласификатора = СтрокаДерева.КВР.Код;
			КонецЕсли;
			...
			Попытка
				...
				// Отмечаем строку как добавленную
				СтрокаДерева.ДобавленоВЖурнал = Истина;
				СтрокаДерева.ДобавитьВЖурнал = Ложь;
	...
	Для Каждого ДочерняяСтрока Из СтрокаДерева.ПолучитьЭлементы() Цикл
		КоличествоДобавлено = КоличествоДобавлено + ОбработатьСтрокуДереваДляДобавленияВЖурнал(ДочерняяСтрока);
	КонецЦикла;
...
Функция ПолучитьТекстЗапросаГруппировки()
	...
	|	ВЫБОР
	|		КОГДА ТЗ.ВидРаботыНаименование = ""Расходники""
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ЭтоРасходник
	|ПОМЕСТИТЬ ВТ_ТЗ
	|ИЗ
	|	&ТЗ КАК ТЗ
	|;
...
Функция СтрокаГотоваКПереносу(СтрокаДерева)
	...
	Возврат СтрокаДерева.Проверено;
...
Функция ПолучитьКолонкиВыгрузки()
	...
	Колонки.Добавить(Новый Структура("Имя, Заголовок, Ширина", "НомерДляСметы", "№ п/п для сметы", 15));
...
Процедура ВывестиДанныеСИерархией(ТабДок, ТаблицаДанных, КолонкиВыгрузки)
	...
	// Счетчики для иерархической нумерации
	НомерУровень1 = 0;
	НомерУровень2 = 0;
	НомерУровень3 = 0;
	НомерУровень4 = 0;
	...
	Для Каждого СтрокаДанных Из ТаблицаДанных Цикл
		...
		Если СтрокаДанных.ОбъектСтроительства <> ТекущийОбъект Тогда
			...
			// Инкремент счетчика уровня 1 и сброс нижних уровней
			НомерУровень1 = НомерУровень1 + 1;
			НомерУровень2 = 0;
			НомерУровень3 = 0;
			НомерУровень4 = 0;
			...
			// Формируем номер для сметы
			НомерДляСметы = Строка(НомерУровень1);

			ВывестиСтрокуУровня(ТабДок, НомерСтроки, НомерПП, НаименованиеОбъекта, НомерДляСметы, 1, КоличествоКолонок);
		...
		Если СтрокаДанных.Помещение <> ТекущееПомещение Тогда
			...
			// Инкремент счетчика уровня 2 и сброс нижних уровней
			НомерУровень2 = НомерУровень2 + 1;
			НомерУровень3 = 0;
			НомерУровень4 = 0;
			...
			Если Не ПустаяСтрока(ТекущееПомещение) Тогда
				...
				// Формируем номер для сметы
				НомерДляСметы = Строка(НомерУровень1) + "." + Строка(НомерУровень2);

				ВывестиСтрокуУровня(ТабДок, НомерСтроки, НомерПП, ТекущееПомещение, НомерДляСметы, 2, КоличествоКолонок);
		...
		Если СтрокаДанных.ТипПомещения <> ТекущийТип Тогда
			...
			// Инкремент счетчика уровня 3 и сброс нижнего уровня
			НомерУровень3 = НомерУровень3 + 1;
			НомерУровень4 = 0;
			...
			Если Не ПустаяСтрока(ТекущийТип) Тогда
				...
				// Формируем номер для сметы
				НомерДляСметы = Строка(НомерУровень1) + "." + Строка(НомерУровень2) + "." + Строка(НомерУровень3) + ".";

				ВывестиСтрокуУровня(ТабДок, НомерСтроки, НомерПП, ТекущийТип, НомерДляСметы, 3, КоличествоКолонок);
		...
		// Инкремент счетчика уровня 4
		НомерУровень4 = НомерУровень4 + 1;

		// Формируем номер для сметы
		НомерДляСметы = Строка(НомерУровень1) + "." + Строка(НомерУровень2) + "." + Строка(НомерУровень3) + "." + Строка(НомерУровень4);

		ВывестиСтрокуДанных(ТабДок, НомерСтроки, НомерПП, СтрокаДанных, НомерДляСметы, КолонкиВыгрузки);
...
&НаСервере
Процедура ВывестиСтрокуУровня(ТабДок, НомерСтроки, НомерПП, ТекстУровня, НомерДляСметы, УровеньИерархии, КоличествоКолонок)

	// Колонка A - номер
	ЯчейкаНомер = ТабДок.Область(НомерСтроки, 1);
	ЯчейкаНомер.Текст = НомерПП;

	// Колонка B - номер для сметы
	ЯчейкаНомерДляСметы = ТабДок.Область(НомерСтроки, 2);
	ЯчейкаНомерДляСметы.Текст = НомерДляСметы;

	// Определяем колонки для текста в зависимости от уровня
	Если УровеньИерархии = 1 Тогда
		// Уровень 1: объединение C-D
		ОбластьТекста = ТабДок.Область(НомерСтроки, 3, НомерСтроки, 4);
		ОбластьТекста.Объединить();
		ОбластьТекста.Текст = ТекстУровня;
	Иначе
		// Уровни 2-3: только колонка C
		ЯчейкаТекст = ТабДок.Область(НомерСтроки, 3);
...
&НаСервере
Процедура ВывестиСтрокуДанных(ТабДок, НомерСтроки, НомерПП, СтрокаДанных, НомерДляСметы, КолонкиВыгрузки)
	...
	Для НомерКолонки = 1 По КолонкиВыгрузки.Количество() Цикл
		...
		Если Колонка.Имя = "НомерПП" Тогда
			Значение = НомерПП;
		ИначеЕсли Колонка.Имя = "НомерДляСметы" Тогда
			Значение = НомерДляСметы;
